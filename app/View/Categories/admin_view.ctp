	...
	<dl>
		...
		<dd>
			<?php #echo h($category['Category']['status']); ?>
                        <?php if($category['Category']['status'] ==1){ echo "Active"; }else{ echo "In-Active";} ?>
			&nbsp;
		</dd>
...
<div class="related">
	...
	<?php foreach ($category['ChildCategory'] as $childCategory): ?>
		<tr>
			...
			<!--td><?php #$childCategory['status']; ?></td-->
                        <td><?php if($childCategory['status'] == 1){echo "Active";}else{ echo "In-Active";} ?></td>
			<td class="actions">
