 ...
 * @var string
 */
	public $validationDomain = 'customers';
 ...
 */
	protected function _setupBehaviors() {
		...
		if (class_exists('SluggableBehavior') && Configure::read('Customers.disableSlugs') !== true) {
	...
	protected function _setupValidation() {
		$this->validatePasswordChange = array(
			...
			'confirm_password' => array(
				'required' => array('rule' => array('compareFields', 'new_password', 'confirm_password'), 'required' => true, 'message' => __d('customers', 'The passwords are not equal.'))),
			'old_password' => array(
				'to_short' => array('rule' => 'validateOldPassword', 'required' => true, 'message' => __d('customers', 'Invalid password.'))));
	}
 ...
 */
	public function verifyEmail($token = null) {
		...
		if ($user === false) {
			throw new RuntimeException(__d('customers', 'Invalid token, please check the email you were sent, and retry the verification link.'));
		}

		$expires = strtotime($user[$this->alias]['email_token_expires']);
		if ($expires < time()) {
			throw new RuntimeException(__d('customers', 'The token has expired.'));
		}
 ...
 */
	public function passwordReset($postData = array()) {
		...
		if (!empty($user) && $user[$this->alias]['email_verified'] == 1) {
			...
			$this->invalidate('email', __d('customers', 'This Email Address exists but was never validated.'));
		} else {
			$this->invalidate('email', __d('customers', 'This Email Address does not exist in the system.'));
		}
		...
		return array(
			...
			'confirm_password' => array(
				'required' => array(
					...
					'message' => __d('customers', 'The passwords are not equal.'))));
	}
	...
	public function validateOldPassword($password) {
		if (!isset($this->data[$this->alias]['id']) || empty($this->data[$this->alias]['id'])) {
			if (Configure::read('debug') > 0) {
				throw new OutOfBoundsException(__d('customers', '$this->data[\'' . $this->alias . '\'][\'id\'] has to be set and not empty'));
			}
		}
 ...
 */
	public function view($slug = null, $field = 'slug') {
		...
		if (empty($user)) {
			throw new NotFoundException(__d('customers', 'The Customer does not exist.'));
		}
		return $user;
	}
 ...
 */
	public function checkEmailVerification($postData = array(), $renew = true) {
		...
		if (empty($user)) {
			$this->invalidate('email', __d('customers', 'Invalid Email address.'));
			return false;
		}

		if ($user[$this->alias]['email_verified'] == 1) {
			$this->invalidate('email', __d('customers', 'This email is already verified.'));
			return false;
		}
 ...
 */
	public function register($postData = array(), $options = array()) {
		$Event = new CakeEvent(
			'Customers.Model.Customer.beforeRegister',
			$this,
			array(
				'data' => $postData,
				'options' => $options
		...
		if ($this->validates()) {
			...
			$Event = new CakeEvent(
				'Customers.Model.Customer.afterRegister',
				$this,
				array(
					'data' => $this->data,
					'options' => $options
 ...
 */
	public function resendVerification($postData = array()) {
		if (!isset($postData[$this->alias]['email']) || empty($postData[$this->alias]['email'])) {
			$this->invalidate('email', __d('customers', 'Please enter your email address.'));
			return false;
		}
		...
		if (empty($user)) {
			$this->invalidate('email', __d('customers', 'The email address does not exist in the system'));
			return false;
		}

		if ($user[$this->alias]['email_verified'] == 1) {
			$this->invalidate('email', __d('customers', 'Your account is already authenticaed.'));
			return false;
		}

		if ($user[$this->alias]['active'] == 0) {
			$this->invalidate('email', __d('customers', 'Your account is disabled.'));
			return false;
		}
 ...
 */
	protected function _beforeRegistration($postData = array(), $useEmailVerification = true) {
		...
		$defaultRole = Configure::read('Customers.defaultRole');
		if ($defaultRole) {
 ...
 */
	public function edit($userId = null, $postData = null) {
		...
		$this->set($user);
		if (empty($user)) {
			throw new NotFoundException(__d('customers', 'Invalid Customer'));
		}
 ...
 */
	...
	public function dataDisplay($userId){
		$defaults = array(
			'contain' => array(),
			'conditions' => array($this->alias . '.id' => $userId));
		return $this->find('first', $defaults);
	}
	
	public function saveAccount($userId, $postData){
		
		if (!empty($postData)) {
			return $result = $this->save($postData, false);
		}
	}
	
	
	
	public $validatePayment = array(
